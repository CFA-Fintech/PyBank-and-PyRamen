...
* Read in `menu_data.csv` and set its contents to a separate list object. (This way, you can cross-reference your menu data with your sales data as you read in your sales data in the coming steps.)
  ...
  * Initialized an empty `menu` list object to hold the contents of `menu_data.csv`.

  * Used a `with` statement and open the `menu_data.csv` by using its file path.

  * Used the `reader` function from the `csv` library to begin reading `menu_data.csv`.

  * Used the `next` function to skip the header (first row of the CSV).
...
* Initialized an empty `report` dictionary to hold the future aggregated per-product results. The `report` dictionary will eventually contain the following metrics:
...
* Then, loop through every row in the `sales` list object.
  ...
  * For each row of the `sales` data, I have set the following columns of the sales data to their own variables:

    * Quantity
    * Menu_Item

  * Performed a quick check if the `sales_item` is already included in the `report`. If not, initialize the key-value pairs for the particular `sales_item` in the report. Then, set the `sales_item` as a new key to the `report` dictionary and the values as a nested dictionary containing the following:
...
* Created a nested loop by looping through every record in `menu`.
